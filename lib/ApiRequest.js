...
var log = (nconf.get('uber_api_debug') ? log.bind(console) : function () {});
...
function getToken(options) {
    ...
    return rest.post(OAUTH_URI, {
        ...
        log('getToken result', result.data);
        if (result.response.statusCode == 200) {
            ...
            log('Uber API authentication failed', result.response.raw);
            throw new Error(result.response);
        }
    }, function(error) {
        log('Uber API authentication error', error.response.raw);
