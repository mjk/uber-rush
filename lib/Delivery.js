...
class Delivery extends EventEmitter {
    ...
    return API.post('deliveries', {
      ...
      if (result && result.response && (result.response.statusCode == 200 || result.response.statusCode == 201)) {
        ...
        throw new Error(result ?
                        'Uber API unable to create delivery. Response: ' + JSON.stringify(result) :
  ...
  getRatings(options) {
    if (this.status !== 'completed' && !options) throw new Error('Cannot get ratings for an order that was not successfully completed (status='+this.status+')');

    return API.get('deliveries/'+(((options||{}).delivery_id)||this.delivery_id)+'/ratings')
    .then(function(result) {
  ...
  rate(options) {
    ...
    if (!options.waypoint || (options.waypoint !== 'pickup' && options.waypoint !== 'dropoff')) throw new Error('Waypoint required: "pickup" or "dropoff"');
