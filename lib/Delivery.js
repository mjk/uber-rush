...
Delivery.prototype.confirm = function(options) {
    ...
    return API.post('deliveries', {
        ...
        if (result.response.statusCode == 200 || result.response.statusCode == 201) {
            ...
            self.updateStatus();
            console.log('Delivery created', self.delivery_id);

            var simulate = nconf.get('simulate') || false;
...
function simulate(delay) {
    ...
    function testStatus(i) {
        if (i > statuses.length) return;
        self.updateStatus(statuses[i]);
        setTimeout(function() { testStatus(++i) }, isNaN(delay) ? 30000 : delay);
    }

    console.log('simulating delivery with a delay of ' + Math.round(delay/1000) + 's between stages');
    testStatus(0);
};
...
Delivery.prototype.updateStatus = function(status) {
    ...
    if (status) {
        // sandbox testing
        console.log('updating status for sandbox testing (delivery_id='+self.delivery_id+'; status='+status+')');
