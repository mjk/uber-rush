  ...
  confirm(options) {
    ...
    return API.post('deliveries', {
      ...
      if (result && result.response && (result.response.statusCode == 200 || result.response.statusCode == 201)) {
        ...
        this.simulating = nconf.get('uber_api_simulate') || false;
        self.log('simulation', this.simulating);
  ...
  updateDeliveryInfo() {
    ...
    return API
      ...
      .catch(err => {
        ...
        if (this.simulating && err.code == 'not_found' && this.pollingFailures === 1) {
          // Polling a simulated delivery that was slow to be created;
          // We'll wait at least once before making a deal of it
          return;
        }
