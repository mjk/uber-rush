...
const DELIVERY_UPDATE_INTERVAL_SECONDS = 30;
...
class Delivery extends EventEmitter {
  ...
  simulate(delay) {
    const statuses = ['en_route_to_pickup', 'at_pickup', 'en_route_to_dropoff', 'at_dropoff', 'completed'];
  ...
  updateStatus(status) {
    ...
    blockUpdate.then(() => {
      ...
      const interval = DELIVERY_UPDATE_INTERVAL_SECONDS * Math.pow(2, self.pollingFailures) * 1000;
  ...
  cancel(status) {
    ...
    return API.post('deliveries/'+(self.delivery_id)+'/cancel').then(() => {
...
function pollDeliveryStatus(delivery) {
  delivery.log('polling delivery status');
